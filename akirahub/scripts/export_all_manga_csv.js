...
async function main() {
  ...
  for (const page of pageList) {
    ...
    for (const m of json.data) {
      if (items.length >= TARGET) break;
      if (['TV Special','Movie','OVA'].includes(m.type)) continue;
  ...
  const header = [
    ...
    'chapters','score','published_from','year',
  ...
  await fs.writeFile('export_all_manga.csv', csv);
  console.log('✅ CSV gerado em export_all_manga.csv');
}
