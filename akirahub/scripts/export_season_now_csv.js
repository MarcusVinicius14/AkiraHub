...
async function main() {
  const BASE_URL  = 'https://api.jikan.moe/v4/seasons/now';
  const PAGE_SIZE = 25;
  const TARGET    = 100;
  ...
  while (rows.length < TARGET) {
    ...
    for (const item of data) {
      ...
      rows.push({
        ...
        episodes:        item.episodes != null ? item.episodes : '',
  ...
  await fs.writeFile('anime_season_now.csv', csv);
  console.log(`✅ anime_season_now.csv gerado com ${rows.length} registros.`);
}
