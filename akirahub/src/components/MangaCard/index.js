...
const MangaCard = ({ manga }) => {
  ...
  // Extraindo o ano do campo "published->from" porque tem um formato diferente
  ...
  return (
    <div className="bg-white rounded-md shadow-sm overflow-hidden flex">
      ...
      <div className="p-3 flex-grow">
        ...
        {chapters !== null && chapters !== undefined ? (
          <p className="text-xs text-gray-600">
            {chapters} {chapters === 1 ? "capítulo" : "capítulos"}
          </p>
        ) : (
          <p className="text-xs text-gray-600">lançando</p>
        )}
        {year && (
          <p className="text-xs text-gray-500 mt-1">Ano: {year}</p>
        )}
